...
if ( ! function_exists( 'ot_type_colorpicker' ) ) {
    ...
    function ot_type_colorpicker( $args = array() ) {
        ...
        $field_position = $field_position ?? 'bottom left';
                ...
                if ( in_array( 'border-settings', $ot_recognized_button_builder_fields ) ) {
                    ...
                    echo '<div id="' . esc_attr( $field_id ) . '_tab_border">';
                        ...
                        echo '<div class="ut-single-options-wrap ut-single-options-wrap-half">';
                            ...
                            $border_style = isset( $field_value['border_style'] ) ? esc_attr( $field_value['border_style'] ) : '';

                            echo '<label for="' . esc_attr( $field_id ) . '-button-border-style">' . __('Button Border Style' , 'unitedthemes') . '</label><br />';

                            echo '<div class="ut-ui-select-field">';

                                echo '<select name="' . esc_attr( $field_name ) . '[border_style]" id="' . esc_attr( $field_id ) . '-border-style" class="ut-ui-form-select ut-ui-select ' . esc_attr( $field_class ) . '">';
                ...
                if ( in_array( 'link-settings', $ot_recognized_button_builder_fields ) ) {
                    ...
                    echo '<div id="' . esc_attr( $field_id ) . '_tab_general">';
                        ...
                        echo '<div class="ut-single-options-wrap ut-single-options-wrap-full">';
                            ...
                            echo '<label for="' . esc_attr( $field_id ) . '_button_extra_class">' . __('Button Class' , 'unitedthemes') . '</label><br />';
                            echo '<input type="text" name="' . esc_attr( $field_name ) . '[b_class]" id="' . esc_attr( $field_id ) . '_button_extra_class" value="' . ( isset( $field_value['b_class'] ) ? esc_attr( $field_value['b_class'] ) : '' ) . '" class="ut-ui-form-input ' . esc_attr( $field_class ) . '" autocomplete="off" />';
                                ...
                                if( isset( $font_settings['font_source'] ) && $font_settings['font_source'] == 'adobe' && !empty( $font_settings['stylesheet_url'] ) ) {
                                    ...
                                    if( $adobe_families = analyze_adobe_font( $font_settings['stylesheet_url'] ) ) {
                                        ...
                                        foreach( $adobe_families as $adobe_family ) {
                                            ...
                                            echo '<option value="' . esc_attr($adobe_family) . '"' . selected($font_family, $adobe_family, false) . '>' . esc_attr($font_family_name) . '</option>';
    ...
    function ot_type_border( $args = array() ) {
        ...
        foreach( $directions as $key => $direction ) {
            ...
            if( in_array( 'border-width', $ut_recognized_border_fields ) )  {
                ...
                $value = !empty( $field_value['border-' . $key . '-width'] ) ? $field_value['border-' . $key . '-width'] : 1;
                ...
                echo '<div class="' . $fields_grid . '">';
                    ...
                    echo '<div class="ut-numeric-slider-outer-wrap">';
                        ...
                        echo '<div class="ut-numeric-slider-wrap">';
                            ...
                            echo '<input autocomplete="off" type="hidden" name="' . esc_attr( $field_name ) . '[border-' . $key . '-width]" id="' . esc_attr( $field_id ) . '-border-' . $key . '-width" class="ut-numeric-slider-hidden-input" value="' . $value . '" data-min="' . esc_attr( $border_min_width ) . '" data-max="' . esc_attr( $border_max_width ) . '" data-step="1">';
