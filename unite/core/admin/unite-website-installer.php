...
class UT_Import
{
    ...
    public function ajax_import_theme_options() {
        ...
        $options = isset($rawdata) ? ot_decode($rawdata) : array();

        // get settings array
        $settings = _ut_theme_options();

        // add contact form 7 designs
        $_skin_settings = get_option('unite-cf7-designs');

        if( is_array( $options['ut_contact_form7_designs'] ) && !empty( $options['ut_contact_form7_designs'] ) ) {

            $_skin_settings = $_skin_settings ? $_skin_settings : array();

            foreach( $options['ut_contact_form7_designs'] as $id => $design ) {

                // d if not exists already
                if( !array_key_exists( $id, $_skin_settings ) ) {

                    $_skin_settings[$id] = $design;

                }

            }

            update_option('unite-cf7-designs', $_skin_settings );
