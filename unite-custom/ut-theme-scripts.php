    ...
    function ut_create_google_font_link() {
        ...
        if( false === ( get_transient( $transient ) ) || $page_overwrite ) {
            ...
            $google_options = array(
                ...
                'ut_background_text_font_type'                  => 'ut_background_text_google_font_style'
            );
        ...
        if( !empty( $query_string ) && is_string( $query_string ) ) { ?>
            ...
            <?php if( ot_get_option('ut_deactivate_google_font_preloads', 'off' ) == 'off' ) : ?>
...
function unite_scripts() {
    ...
    /* Adobe Fonts  */
    if( $adobe_fonts = get_option('unite_adobe_custom_fonts', false ) ) {

        foreach( $adobe_fonts as $key => $adobe_font ) {
	...
	wp_register_script(
		'ut-scroll-dots-js',
		THEME_WEB_ROOT . '/js/scrolldots/ut-scrolldots.min.js',
		array('jquery'),
		UT_THEME_VERSION,
		true
	);
    ...
    wp_register_script(
        'ut-noise',
        THEME_WEB_ROOT . '/js/noise/noise.min.js',
        array(),
        '1.0',
        true
    );
	...
	if( ot_get_option('ut_use_image_loader') == 'on' && !isset( $_GET['vc_editable'] ) ) {
		...
		if( $loader_match ) :
			...
			$loader_settings = array( 
                ...
                'deactivate_image_loading'  => ot_get_option( 'ut_deactivate_image_loader_preloading' , 'off' ),
			);
	...
	if( ut_return_hero_config( 'ut_hero_overlay_effect', 'off' ) == 'on' ) {
		...
		if( apply_filters( 'ut_show_hero', false ) &&
            ut_return_hero_config('ut_hero_overlay_effect') == 'on' &&
            ( ut_return_hero_config( 'ut_hero_overlay_effect_style' ) == 'dots' || ut_return_hero_config( 'ut_hero_overlay_effect_style' ) == 'bubbles' ) ) {
	...
	/* Section Scroll */
	if( ut_page_option( 'ut_activate_section_scroll', 'off', 'ut_' ) == 'on' ) {

	    wp_enqueue_script( 'ut-scroll-dots-js' );
...
function unite_require_js() {
    ...
    $params = array(
        ...
        'auroraJS' => array(
            'scriptUrl' => array(
                unite_script_path() . '/js/noise/noise.min.js',
                unite_script_path() . '/js/noise/aurora' . $min . '.js',
            ),
        ),
        ...
        'swirlJS' => array(
            'scriptUrl' => array(
                unite_script_path() . '/js/noise/noise.min.js',
                unite_script_path() . '/js/noise/swirl' . $min . '.js',
            ),
        ),
        ...
        'shiftJS' => array(
            'scriptUrl' => array(
                unite_script_path() . '/js/noise/noise.min.js',
                unite_script_path() . '/js/noise/shift' . $min . '.js',
            ),
        ),
        ...
        'pipelineJS' => array(
            'scriptUrl' => array(
                unite_script_path() . '/js/noise/noise.min.js',
                unite_script_path() . '/js/noise/pipeline' . $min . '.js',
            ),
        ),
        ...
        'coalesceJS' => array(
            'scriptUrl' => array(
                unite_script_path() . '/js/noise/noise.min.js',
                unite_script_path() . '/js/noise/coalesce' . $min . '.js',
            ),
        ),
