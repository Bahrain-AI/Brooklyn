        ...
        public function menu_setup() {
            ...
            foreach ( $locations as $location => $description ) {
                ...
                if( function_exists('pll_languages_list') ) {
                    ...
                    if( ot_get_option( 'ut_header_layout', 'default' ) == 'default' ) {

                        if( ( 'primary' == $location || 'secondary' == $location ) && isset( $menu_locations[ $location ] ) && $menu_locations[ $location ] == $nav_menu_selected_id ) {
                        ...
                        foreach( pll_languages_list() as $language ) {
                            ...
                            $_location = $location . '___' . $language;

                            if( ( 'primary' == $location || 'secondary' == $location ) && isset( $menu_locations[ $_location ] ) && $menu_locations[ $_location ] == $nav_menu_selected_id ) {
...
if( !class_exists('WP_UT_Menu_Walker') ) :
    ...
    class WP_UT_Menu_Walker extends Walker_Nav_Menu {
        ...
        public function start_mega_menu_section( &$output, $depth = 0, $args = array() ) {
            ...
            /* merge class names */
            $classes = apply_filters('ut_theme_menu_item_classes', $classes, $this->item );
        ...
        public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {            
	            ...
	            foreach( $nav_menu_items as $nav_menu_item ) {
                    ...
                    if( isset( $nav_menu_item->menu_item_parent ) && $nav_menu_item->menu_item_parent == $item->ID ) {
                        ...
                        if( !empty( $nav_menu_item->url ) ) {
	                        ...
	                        if( ( isset( $item_url['host'] ) && isset( $current_url['host'] ) && ( $item_url['host'] == $current_url['host'] ) ) && ( isset( $item_url['path'] ) && isset( $current_url['path'] ) && rtrim($item_url['path'], '/') . '/' ==  rtrim($current_url['path'], '/') . '/' ) && !empty( $item_url['fragment'] ) ) {
            ...
            /* merge class names */
            $classes = apply_filters('ut_theme_menu_item_classes', $classes, $item );
