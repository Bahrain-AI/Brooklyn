...
if( !class_exists( 'UT_Custom_CSS' ) ) {	
    ...
    class UT_Custom_CSS {
        ...
        /**
         *  Assign Global Post ID and Accent Color
		 */
        ...
        public function css_background( $object , $background_settings ) { 
            ...
            foreach( $background_settings as $key => $value) {            
                ...
                if( in_array( $key , $key_exceptions ) ) {
                    ...
                    if( $skip_fixed && $key == 'background-attachment' ) {
        ...
        public function create_button( $selector, $button_settings = array() ) {
            ...
            $button .= $selector . '{';
				...
				if( !empty( $button_settings['border_style'] ) ) {
                    $button .= 'border-style:' . $button_settings['border_style'] . ' !important;';
                }
        ...
        public function minify_css( $css ) {
            ...
            if( apply_filters( 'ut_minify_assets', true ) && !empty( $css ) ) {
        ...
        public function get_font_face() {
			...
			if( !empty( $fonts ) ) {
				...
				foreach ( $fonts as $font ) {
				    ...
				    if( isset( $font->links['font_source'] ) && $font->links['font_source'] == 'adobe' ) {
				        continue;
                    }
					...
					@font-face {
						...
						font-style: normal;
						font-weight: normal;
                        <?php if( !empty( $font->links['ascent_override'] ) ) : ?>ascent-override: <?php echo esc_attr( $font->links['ascent_override'] ); ?>;<?php endif; ?>
					}
        ...
        public function font_style_css( $settings, $exception = array(), $important = false ) {
            ...
            if( array_key_exists('font-type', $settings ) && $settings['font-type'] == 'ut-google' && isset( $settings['google-font-style']['font-family'] ) ) {
				...
				$font = $settings['selector'] . ' {';
					...
					if( array_key_exists( 'google-font-style', $settings ) && array_key_exists( 'line-height', $settings['google-font-style'] ) && !empty( $settings['google-font-style']['line-height']) ) {
					...
					if( array_key_exists( 'google-font-style', $settings ) && array_key_exists( 'letter-spacing', $settings['google-font-style'] ) &&  !empty( $settings['google-font-style']['letter-spacing'] ) ) {

						// fallback letter spacing
						if( (int)$settings['google-font-style']['letter-spacing'] >= 1 || (int)$settings['google-font-style']['letter-spacing'] <= -1 ) {

						    $settings['google-font-style']['letter-spacing'] = (int)$settings['google-font-style']['letter-spacing'] / 100;
                ...
                if( array_key_exists('font-style', $settings ) && !empty( $settings['font-style']['font-style'] ) && array_key_exists( $settings['font-style']['font-style'], $this->theme_font_styles ) ) {
                ...
                foreach( ot_recognized_breakpoints() as $key => $breakpoint ) {
			        ...
			        if( !empty( $font_settings[$attribute . '-responsive'][$key] ) && $font_settings[$attribute . '-responsive'][$key] != 'auto' ) {
			            ...
			            if( in_array( $attribute, array('font-size', 'line-height') ) ) {
			                ...
			                if( $font_settings[$attribute . '-responsive'][$key] != '-0.21' ) {
		...
		public function responsive_font_style_single( string $selector, $font_settings, string $attribute, $important = false ) {
		    ...
		    if( is_array( $font_settings ) && !empty( $font_settings ) ) {
				...
				echo $this->responsive_font_style( $selector, $font_settings );
					...
					$font .= $object . ' {';
						...
						if( !empty( $ut_global_google_headline_font_style['letter-spacing'] ) ) {
							...
							if( $ut_global_google_headline_font_style['letter-spacing'] >= 1 || $ut_global_google_headline_font_style['letter-spacing'] <= -1 ) {
							    ...
							    $ut_global_google_headline_font_style['letter-spacing'] = (int)$ut_global_google_headline_font_style['letter-spacing'] / 100;
