...
if( ut_return_hero_config('ut_hero_rain_effect' , 'off') == 'on' ) {
    ...
    if( unite_mobile_detection()->isTablet() ) {
         ...
         $_ut_hero_image = ut_collect_option('ut_hero_image_tablet', $ut_hero_image );
         $ut_hero_image = !empty( $_ut_hero_image[ 'background-image' ] ) && is_array( $_ut_hero_image ) ? $_ut_hero_image[ 'background-image' ] : $ut_hero_image;

    }

    if( unite_mobile_detection()->isMobile() && !unite_mobile_detection()->isTablet() ) {

        $_ut_hero_image = ut_collect_option('ut_hero_image_mobile', $ut_hero_image );
        $ut_hero_image = !empty( $_ut_hero_image[ 'background-image' ] ) && is_array( $_ut_hero_image ) ? $_ut_hero_image[ 'background-image' ] : $ut_hero_image;
...
<section id="ut-hero" class="hero ha-waypoint parallax-section parallax-background <?php echo implode( " " , $hero_classes ); ?>" data-animate-up="ha-header-hide" data-animate-down="ha-header-hide">
    ...
    <?php endif; ?>
        ...
        if( ut_return_hero_config('ut_hero_overlay_effect') == 'on') : ?>
            ...
            <?php if( ut_return_hero_config( 'ut_hero_overlay_effect_style' ) == 'dots' || ut_return_hero_config( 'ut_hero_overlay_effect_style' ) == 'bubbles' ) : ?>

                <canvas data-strokecolor="<?php echo ut_hex_to_rgb( $ut_effect_color ); ?>" id="ut-animation-canvas"></canvas>

            <?php else : ?>

               <div id="ut-hero-animation" class="bklyn-overlay-effect" data-effect="<?php echo  ut_return_hero_config( 'ut_hero_overlay_effect_style' ); ?>" data-effect-config='<?php echo json_encode( ut_create_overlay_effect_settings( array(
                    'bklyn_overlay_effect_base_color'       => ut_return_hero_config('ut_hero_overlay_effect_color'),
                    'bklyn_overlay_effect_background_color' => ut_return_hero_config('ut_hero_overlay_effect_background_color'),
                    'bklyn_overlay_effect_color_range'      => ut_return_hero_config('ut_hero_overlay_effect_color_range')
               ) ) ); ?>'></div>
        ...
        <div class="grid-container">
            ...
            <!-- start hero-holder -->
            <div class="hero-holder ut-split-hero grid-100 mobile-grid-100 tablet-grid-100 <?php echo $ut_hero_split_content_type == 'form' ? 'ut-hero-highlighted-with-form' : '' ; ?> <?php echo ut_return_hero_config('ut_hero_style' , 'ut-hero-style-1'); ?> hero-holder-align-items-<?php echo ut_return_hero_config('ut_hero_v_align', 'middle'); ?>">
