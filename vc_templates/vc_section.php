...
global $section_is_contact_section, $current_section_id;
...
$el_class = $el_name = $full_height = $hide_on_desktop = $hide_on_tablet = $hide_on_mobile = $full_width = $flex_row = $columns_placement = $content_placement = $css = $el_id = $video_bg = $video_bg_url = $css_animation = '';
$disable_element = $disable_scroll_dots = $output = $after_output = $video_background = '';
...
$bklyn_activate_background_text = $bklyn_background_text = $section_separator_top = $section_separator_bottom = false;
...
if( $el_name ) {
    ...
    $wrapper_attributes[] = 'data-section-title="' . esc_attr( $el_name ) . '"';

}

if ( 'yes' === $disable_scroll_dots ) {

    $wrapper_attributes[] = 'data-hide-from-dots="true"';
...
// attach video background markup and script
...
if( $bklyn_activate_background_text == 'on' && !empty( $bklyn_background_text ) ) {
    ...
    $output .= ut_create_background_text_template( $atts, $el_id, 'vc_section' );
...
// custom css
if( !empty( $custom_css_style ) || !empty( $parallax_css_style ) || !empty( $background_text_css_style ) ) {
